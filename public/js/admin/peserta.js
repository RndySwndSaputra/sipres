(() => {
  ...
  const loadingOverlay = document.getElementById('pesertaLoading');
  ...
  const getEventStatus = (mulai, selesai) => {
    try {
      ...
      if (!endStr) return { label: 'Akan Datang', class: 'status-upcoming' };
  ...
  const render = () => {
    ...
    const filtered = acara.filter(a => {
      ...
      let matchDate = true;
      if (valStart || valEnd) {
          // Ambil bagian tanggal saja (YYYY-MM-DD) dari waktu mulai acara
      ...
      return `
        <div class="acara-card" data-id="${a.id_acara}">
          ...
          <div class="card-footer">
            <button class="btn btn-primary btn-view" data-action="view" title="Lihat daftar peserta">
  ...
  acaraGrid.addEventListener('click', (e) => {
    ...
    if (event) {
      window.location.href = `/admin/peserta/view/${id}`;
    }
  });

  if (loadingOverlay) loadingOverlay.hidden = false;
  fetch('/admin/peserta/acara', { headers: { 'Accept': 'application/json' } })
    .then(res => res.json())
    .then(json => {
      acara = (json && json.success && Array.isArray(json.data)) ? json.data : [];
      render();
